...
function facilityMatchesCareType(
  requiredCareType: string,
  facilityCareType: string,
): boolean {
  if (requiredCareType === "stationary") {
    return (
      facilityCareType === "Stationary" ||
      facilityCareType === "Stationary & Ambulatory"
    );
  }
  if (requiredCareType === "ambulatory") {
    return (
      facilityCareType === "Ambulatory" ||
      facilityCareType === "Stationary & Ambulatory"
    );
  }
  return false;
...
function patientZipWithinFacilityRange(
  patientZip: number,
...
export function findMatchingFacility(careType: string, zipCode: string) {
  ...
  const facilitiesServingPatient = careTypeMatchedFacilities.filter(
    (facility) => patientZipWithinFacilityRange(patientZip, facility),
  );
  ...
  const facilitiesSortedByDistance = facilitiesServingPatient.sort(
    (a, b) =>
      calculateDistance(patientZip, a.facilityZipCode) -
      calculateDistance(patientZip, b.facilityZipCode),
  );

  const nearestFacilityWithCapacity = facilitiesSortedByDistance.find(
    (facility) => facility.capacity === "Available",
  );
  ...
  if (
    anyAvailableFacility &&
    calculateDistance(patientZip, anyAvailableFacility.facilityZipCode) <= 3000
  ) {
